...
> 本文档是 `sfwork` 工作空间的总体说明，面向需要同时改动 Privahub 前端、Privahub 后端、Kuscia 或 SecretFlow 算法组件的开发者。内容综合了 `AGENTS.md`、`PROJECT_SUMMARY.md`、`docs/前后端Kuscia及SecretFlow算法模块联调与Bug定位指南.md` 以及各隐私计算组件文档，力求覆盖架构、技术栈、构建方式、运行端口和常见问题的完整信息。
...
| **Privahub 后端** | Go 1.25 + Gin + GORM | Web 管理控制台后端，负责业务逻辑和与 Kuscia 交互 | `privahub/` |
| **Privahub 前端** | TypeScript + React 18 + Vite 5 | Web 管理控制台 UI | `privahub/web/` |

> 注意：根目录下还有一个 `privahub/frontend-src/` 的历史副本，当前活跃开发在 `privahub/web/`。
...
```text
Privahub 前端 (React/Vite，localhost:8000)
        │  REST /api/v1alpha1/*
        ▼
Privahub 后端 (Go，localhost:8080/9001)
...
- **后端**：`privahub/internal/service/` 处理业务逻辑，`privahub/pkg/kuscia/` 通过 gRPC 调用 KusciaAPI，把图转换成 `ProjectJob` 并生成 `KusciaJob` 的 `TaskInputConfig`。
...
| 服务 | 端口 | 说明 |
|------|------|------|
| KusciaAPI gRPC | 18083 | Privahub 后端连接（Docker 模式映射；配置沿用历史 `privahub` 命名） |
...
**项目描述**
Privahub 是 Web 管理控制台后端，基于 Go 重新实现，处理用户、项目、图、作业、数据表等业务逻辑，并通过 KusciaAPI gRPC 把作业下发给 Kuscia。

**主要语言与技术栈**

- Go 1.25
- Gin HTTP 框架
- GORM + SQLite（默认）/ MySQL
- Viper 配置管理，`config/privahub*.yaml` 多 profile
- gRPC 调用 KusciaAPI（`privahub/pkg/kuscia`）
- Zap 结构化日志
- JWT 认证
...
|------|------|
| `cmd/server/` | 主服务入口 |
| `internal/controller/http/` | REST 接口处理器（`/api/v1alpha1/*`） |
| `internal/service/` | 业务逻辑 |
| `internal/dao/` | GORM 模型、迁移、仓库 |
| `internal/wire/` | 依赖注入 wiring |
| `pkg/kuscia/` | KusciaAPI gRPC 客户端 |
...
# 运行测试（需要 CGO 以支持 SQLite）
CGO_ENABLED=1 go test ./...

# 构建后端二进制（需要 CGO 以支持 SQLite）
...
- `internal/controller/http/v1/graph_handler.go`：REST 接口 `/api/v1alpha1/graph/*`
- `internal/service/graph_service.go`：图与作业业务逻辑
- `pkg/kuscia/job.go`：生成 `CreateJobRequest` 并提交给 Kuscia
...
**项目描述**
Privahub 的 Web UI，提供数据管理、训练流 DAG 编辑、作业监控和结果可视化。

**主要语言与技术栈**

- Node.js >= 18（建议 >= 22.13 以使用 pnpm 11.7.0）
- pnpm 11.7.0（由 `packageManager` 字段管理）
- React 18 + Vite 5
- Tailwind CSS
- TypeScript 5.x
- Zustand 状态管理
...
| 目录 | 用途 |
|------|------|
| `apps/privahub/` | 主 Privahub Web 应用 |
| `packages/api-client/` | `@privahub/api-client` API 类型与 mock |
| `packages/dag-next/` | `@privahub/dag-next` DAG 画布引擎 |
| `packages/design-system/` | `@privahub/design-system` 组件库 |
| `packages/utils/` | `@privahub/utils` 共享工具 |

**常用命令**

```bash
cd /home/charles/code/sfwork/privahub/web
corepack enable                  # 启用项目指定 pnpm 版本
corepack pnpm install            # 安装依赖并构建共享包
corepack pnpm --filter @privahub/app dev     # 开发服务器 http://localhost:8000
corepack pnpm --filter @privahub/app build   # 生产构建
...
- `privahub/proto/secretflow/spec/v1/*.proto` 的 `package` 从 `secretflow.spec.v1` 改为 `secretflow_spec.v1`，与 SecretFlow Python 包一致。
...
- 组件列表 `privahub/config/components/secretflow.json` 与 i18n 文件已包含 `privacy/l_diversity`。
...
| 服务 | 端口 | 说明 |
|------|------|------|
| Privahub 前端 dev server | 8000 | Vite dev，/api 代理到后端 8080 |
| Privahub 后端 HTTP | 8080 | Go HTTP 服务 |
| Privahub 内部 API | 9001 | 集群内部端口（无认证） |
| KusciaAPI gRPC | 18083 | Privahub 后端连接（Docker 模式映射） |
| Kuscia Gateway 公共 | 18080 | 外部访问 |
| Kuscia Envoy 跨域网关 | 18080 | 数据面通信 |

### 5.3 后端连接 Kuscia 的配置

Docker 开发模式下使用 dev profile（`PRIVAHUB_PROFILE=dev`），`privahub/config/privahub-dev.yaml` 已配置：
...
非 Docker 模式下直接使用 `privahub/config/privahub.yaml`，其中 `api_port` 为 `8083`。
...
| 构建 Privahub 后端 | `cd privahub && CGO_ENABLED=1 go build -o bin/privahub ./cmd/server` |
| 测试 Privahub 后端 | `cd privahub && CGO_ENABLED=1 go test ./...` |
| 构建隐私计算镜像 | `cd secretflow/docker/privacy-dev && docker build . -f Dockerfile -t secretflow/sf-privacy-dev:1.15.0.dev-privacy` |
| 前端安装+构建共享包 | `cd privahub/web && corepack enable && corepack pnpm install` |
| 前端开发服务器 | `cd privahub/web && corepack pnpm --filter @privahub/app dev` |
...
| 功能 | 端点 | 关键类 |
|------|------|--------|
| 创建图 | `/api/v1alpha1/graph/create` | `internal/controller/http/v1/graph_handler.go` |
| 启动图 | `/api/v1alpha1/graph/start` | `internal/service/graph_service.go` |
| 查询节点状态 | `/api/v1alpha1/graph/node/status` | `internal/controller/http/v1/graph_handler.go` |
...
| 现象 | 根因 | 解决 |
|------|------|------|
| Kuscia 任务报 `ParseError: Can not find message descriptor by type_url: type.googleapis.com/secretflow.spec.v1.IndividualTable` | `.proto` package 不一致 | 检查 Kuscia / SecretFlow / Privahub 共享 proto 的 `package` 是否统一为 `secretflow_spec.v1` |
...
2. **Privahub 后端侧**
   - 确保 `privahub/pkg/kuscia` 中的协议转换与新 `NodeEvalParam` 兼容。
   - `CGO_ENABLED=1 go test ./...` 通过。
3. **Privahub 前端侧**
