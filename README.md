...
sfwork 是 SecretFlow 隐私计算生态的本地开发工作区，聚合了 Kuscia、SecretFlow、Privahub（含前端）三个核心仓库，并提供统一的文档与编排脚本。
...
│   │   ├── 02-前端开发/          # 历史 Privahub 前端开发指南（Privahub 前端见 privahub/web/）
│   │   ├── 03-后端开发/          # 历史 Privahub 后端开发指南（Privahub 后端见 privahub/）
...
privahub/       # Web 管理控制台（Go 后端 + React/Vite 前端 privahub/web/）
```
...
| 项目 | 语言 | 用途 |
|------|------|------|
| **privacy-java-sdk** | Java 17 | Java/Privahub 后端本地隐私 SDK |
...
| **Privahub Backend** | Go 1.25 | Gin、gRPC、GORM、SQLite/MySQL、Viper |
| **Privahub Frontend** | TypeScript 5.x | React 18、Vite 5、Tailwind CSS、Zustand |
...
| 服务 | 端口 | 说明 |
|------|------|------|
| Privahub 前端开发服务器 | 8000 | Vite dev，代理 `/api` 到后端 |
| Privahub 后端 HTTP | 8080 | Go HTTP 服务 |
| Privahub 内部 API | 9001 | 集群内部端口（无认证） |
| Kuscia API gRPC | 8083 | 非 Docker 模式；Docker 模式映射为 18083 |
| Kuscia Envoy 跨域网关 | 1080 | Docker 模式映射为 18080；非 Docker 模式使用 80 |
...
| `KUSCIA_GW_ADDRESS` | `127.0.0.1:18080` | Kuscia Envoy 网关地址（Docker 模式映射） |
...
```bash
cd privahub
CGO_ENABLED=1 go build -o bin/privahub ./cmd/server  # 构建二进制
CGO_ENABLED=1 go test ./...                            # 运行单元测试
make image                     # 构建 Docker 镜像（如 Makefile 支持）
```
...
```bash
cd privahub/web
corepack enable                # 启用项目指定的 pnpm 版本
corepack pnpm install          # 安装依赖
corepack pnpm --filter @privahub/app build   # 构建生产包
corepack pnpm --filter @privahub/app dev       # 启动开发服务器（http://localhost:8000）
...
| 前端开发 | [docs/doc-center/02-前端开发/](./docs/doc-center/02-前端开发/)、[AGENTS.md §4.4](./AGENTS.md#44-privahub-frontend) |
| 后端开发 | [docs/doc-center/03-后端开发/](./docs/doc-center/03-后端开发/)、[AGENTS.md §4.3](./AGENTS.md#43-privahub-backend) |
...
- **secretpad-frontend-dev** —— 前端开发工作流（Privahub 前端，源码在 `privahub/web/`）
- **secretpad-backend-dev** —— 后端开发工作流（Privahub 后端，源码在 `privahub/`）
...
- **mTLS**：Kuscia 在生产环境使用 mTLS 进行跨域通信；开发模式使用 `kuscia.protocol=notls`（由 `privahub/config/privahub*.yaml` 控制）
- **证书**：Privahub 开发模式默认 notls，无需生成 Java JKS 证书；生产环境请配置 TLS 证书并妥善保管
- **认证**：Privahub 使用 JWT + 数据库用户/令牌机制，开发环境默认账号为 `admin` / `12345678`
...
- **敏感文件**：前端 `.env` 代理配置已加入 gitignore；Kuscia 证书目录 `.local-kuscia/var/certs/` 仅限本地
...
- **前端 ↔ 后端**：REST JSON under `/api/v1alpha1/*`，业务模型与 DTOs 位于 `privahub/internal/service`、`privahub/internal/controller/http` 等包
- **后端 ↔ Kuscia**：gRPC 通过 `privahub/pkg/kuscia` 客户端与 KusciaAPI 通信，连接参数由 `privahub/config/privahub*.yaml` 或 `PRIVAHUB_*` 环境变量控制
...
3. **修改后端代码**：`cd privahub && CGO_ENABLED=1 go build -o bin/privahub ./cmd/server`，重启后端
4. **修改前端代码**：`cd privahub/web && corepack enable && corepack pnpm --filter @privahub/app dev` 支持热重载
